...
class SafeConceptDriftStream(ConceptDriftStream):
    """Custom ConceptDriftStream that does not crash when calculating the probability for a drift.
    ...
    def __iter__(self):
        ...
        while True:
            ...
            v_clamped = max(min(v, 100), -100)
            probability_drift = 1.0 / (1.0 + np.exp(v_clamped))
            try:
